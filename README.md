...
- **下载 v1.0.1（2026-09 重建版）**：前往 [POICollector v1.0.1 Release](https://github.com/joey6657-6657/POICollector/releases/tag/v1.0.1)，在 Assets 中下载 `POICollector.exe`。本版本为重建版（修复 POI 类型编码表、大多边形采集与错误诊断），此前下载过旧 v1.0.1 构建的用户请重新下载。
...
大范围 + 勾选整个 POI 大类时，软件会把区域切成数百个小网格逐个采集，总请求数较大；个人开发者 Key 每秒限 3 次请求，整体耗时可能达数分钟到数十分钟，属正常现象。v1.0.1 起运行日志会定期播报「已探测 N 个子网格」，看到它在增长就说明采集仍在推进。建议使用 2~3 个 Key（英文逗号分隔填入）自动轮询分摊限流。
...
「Key 无效（10001）」表示 Key 不存在或类型错误：请到[高德控制台](https://console.amap.com/dev/key/app)核对 Key 是否存在，且「服务平台」必须为 **Web服务**（Web端JS / Android / iOS 类型的 Key 不能用于本软件）。无效 Key 会在日志中明确报错并立即终止，不再静默返回 0 条；若采集完成但结果为 0 条，日志会输出常见原因提示（Key 类型 / 坐标顺序为「经度,纬度」且为 GCJ-02 / 类型关键词匹配）。若日志出现「响应不是有效 JSON」，通常是代理、VPN 或安全软件拦截了请求，可按日志中的响应片段判断。

**Q：勾选了 POI 类型就查不到数据（0 条）？**

2026-09 之前的构建内置的 POI 分类编码表存在缺陷：0 开头的编码丢失了前导零（如餐饮服务 `050000` 被存成 `50000`、汽车维修 `030000` 被存成 `30000`），按类型查询在高德匹配不到任何数据。请更新到 2026-09 重建版，更新后重新勾选一次 POI 类型即可。

**Q：多边形搜索报「响应不是有效 JSON」或返回空？**

顶点过多（数百个）时 GET 请求 URL 会超过服务端长度限制，服务器直接拒收并返回空响应（实测 HTTP 413）。新版本会自动改用包围盒矩形查询、完成后在本地按多边形边界过滤，并自动闭合首尾顶点；同时也建议尽量精简多边形顶点。

**Q：采集可以指定行政区划吗？城市框里填什么？**

「城市」支持城市名（如 `苏州`）、citycode 或 adcode（行政区划编码，如 `320500`）均可；填入后建议勾选「仅限城市内」以提高数据准确度。

**Q：首次运行被 SmartScreen 拦截？**

软件未购买代码签名证书，首次运行会提示拦截，点击「更多信息 → 仍要运行」即可；请务必只从本仓库 Release 下载并核对 SHA-256。
...
├── requirements-lock.txt       # v1.0.1 已验证的完整环境
